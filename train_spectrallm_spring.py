...
from models.SpectraLLM import SpectraLLM
...
def train_model(args):
    ...
    print("="*70)
    print("SpectraLLM Training on Spring Dataset")
    print("="*70)
    ...
    model = SpectraLLM(configs, target_indices=args.target_indices).to(device)
    ...
    checkpoint_path = os.path.join(args.checkpoint_dir, 'best_model.pth') if args.checkpoint_dir else 'checkpoint_spectrallm_spring.pth'
    ...
    for epoch in range(args.train_epochs):
        ...
        for batch_x, batch_y, _, _ in train_loader:
            ...
            preds, freq_weights, outputs = model(batch_x)
            
            total_loss = sum(
                criterion(preds[i], batch_y[:, -args.pred_len:, idx].unsqueeze(-1)) * task_weights[idx]
    ...
    print(f"Test MAE (original scale): {avg_mae:.3f}")
